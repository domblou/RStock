...
def _promotion_reasons(
    ...
    reasons: list[str] = []
    if pd.isna(threshold):
        reasons.append("Seuil calibré absent")
    elif str(selection.get("status", "")).lower() != "selected":
        reasons.append("Seuil non sélectionné")
    ...
    for column in (
        ...
        if pd.isna(value):
            reasons.append(f"{column} absente")
        else:
...
def threshold_promotion_guidance(
    ...
    for _, row in table.iterrows():
        ...
        if blockers:
            ...
            continue
        statuses.append("Candidat")
        reasons.append("Tous les critères passent")

    table["Statut promotion"] = statuses
    table["Raison"] = reasons
    ordered = [
        *PROMOTION_GUIDANCE_COLUMNS,
...
def filter_threshold_calibration_results(
    ...
    *,
    ...
    if promotion_status in {"Candidat", "Non candidat"}:
